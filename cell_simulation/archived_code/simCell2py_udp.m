...
function [vk,rck,hk,zk,sik,OCV] = simCell2py_udp(ik,T,deltaT,model,z0,iR0,h0, udp_port_in, udp_port_out)

    udp_conn = udpport("LocalPort", udp_port_in);

    while udp_conn.NumBytesAvailable == 0; end % blocking wait

%     data = jsondecode(native2unicode(read(udp_conn,udp_conn.NumBytesAvailable))); % Read and decode commands from python server
% 
...
%     end
    ...
    write(udp_conn, jsonencode(send2Py), "char", "127.0.0.1", udp_port_out);
